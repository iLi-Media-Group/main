-- Final RLS fix script with conditional column checking
...
-- 2. sync_request_messages table (conditional policies based on column existence)
-- Enable RLS
...
-- Create policies for sync_request_messages with conditional logic
DO $$
BEGIN
    -- Check if producer_id column exists
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'sync_request_messages' AND column_name = 'producer_id'
    ) THEN
        -- Create policies with producer_id
        EXECUTE 'CREATE POLICY "sync_request_messages_select_policy" ON sync_request_messages
            FOR SELECT USING (
                EXISTS (
        ...
        EXECUTE 'CREATE POLICY "sync_request_messages_insert_policy" ON sync_request_messages
            FOR INSERT WITH CHECK (
                EXISTS (
                    SELECT 1 FROM custom_sync_requests csr 
                    WHERE csr.id = sync_request_messages.sync_request_id 
        ...
        EXECUTE 'CREATE POLICY "sync_request_messages_update_policy" ON sync_request_messages
            FOR UPDATE USING (
        ...
        EXECUTE 'CREATE POLICY "sync_request_messages_delete_policy" ON sync_request_messages
            FOR DELETE USING (
                producer_id = auth.uid()
            )';
    ELSE
        -- Create policies without producer_id (fallback)
        EXECUTE 'CREATE POLICY "sync_request_messages_select_policy" ON sync_request_messages
            FOR SELECT USING (
                EXISTS (
                    SELECT 1 FROM custom_sync_requests csr 
