-- Safe SQL script to create mood tables without affecting existing data
...
-- 11. Create a view to help with the transition (shows both old and new mood data)
...
WHERE t.moods IS NOT NULL AND t.moods != '{}' AND t.moods != '[]'
GROUP BY t.id, t.title, t.moods
ORDER BY t.title;

-- 12. Create a function to get track moods in the new format
...
-- Success message
SELECT 'Mood tables created successfully! You can now use the new mood structure.' as status;
