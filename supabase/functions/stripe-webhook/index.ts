    ...
    if (isSubscription) {
      ...
      try {
        ...
        if (metadata?.proposal_id) {
          ...
          const { data: producerData, error: producerError } = await supabase.from('profiles').select('email').eq('id', proposalData.track.producer_id).single();
          if (producerError) {
            console.error('Error fetching producer email:', producerError);
            return;
          }
          // Get client email for notification
          const { data: clientData, error: clientError } = await supabase.from('profiles').select('email').eq('id', proposalData.client_id).single();
          if (clientError) {
        ...
        if (trackId && customerData?.user_id) {
          ...
          const { data: trackData, error: trackError } = await supabase.from('tracks').select('id, producer_id').eq('id', trackId).single();
          if (trackError) {
