...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Find active subscription (status: active or trialing)
    ...
    // Resume the subscription (set cancel_at_period_end to false)
    const resumed = await stripe.subscriptions.update(subscription.id, { cancel_at_period_end: false });

    // Update DB (optional: set status to 'active')
    await supabase
      .from('stripe_subscriptions')
      .update({ status: 'active', cancel_at_period_end: false })
      .eq('subscription_id', subscription.id);

    // Update profile to clear pending downgrade fields
