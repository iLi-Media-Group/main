...
-- This migration adds all the missing columns for the sync licensing quiz
...
-- Add new instrument fields (if they don't exist)
ALTER TABLE producer_applications 
...
-- Add sample use field (if it doesn't exist)
ALTER TABLE producer_applications 
ADD COLUMN IF NOT EXISTS sample_use TEXT;

-- Add loop use field (if it doesn't exist)
ALTER TABLE producer_applications 
ADD COLUMN IF NOT EXISTS loop_use TEXT;

-- Add splice use field (if it doesn't exist)
ALTER TABLE producer_applications 
ADD COLUMN IF NOT EXISTS splice_use TEXT;

-- Add business entity field (if it doesn't exist)
ALTER TABLE producer_applications 
ADD COLUMN IF NOT EXISTS business_entity TEXT;

-- Add pro affiliation field (if it doesn't exist)
ALTER TABLE producer_applications 
ADD COLUMN IF NOT EXISTS pro_affiliation TEXT;

-- Add additional info field (if it doesn't exist)
ALTER TABLE producer_applications 
ADD COLUMN IF NOT EXISTS additional_info TEXT;

-- Add status and review fields (if they don't exist)
ALTER TABLE producer_applications 
