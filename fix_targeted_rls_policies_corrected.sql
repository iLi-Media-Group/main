-- Targeted RLS Fix for Specific Tables (CORRECTED)
...
-- ============================================
-- Based on FK: producer_id -> profiles.id
...
-- ============================================
-- Based on FK: user_id -> clients.id, subscription_id -> subscriptions.id
...
CREATE POLICY "Users can view their own membership history" ON membership_history
    FOR SELECT USING (
        user_id = (SELECT id FROM clients WHERE user_id = auth.uid())
    );
...
-- ============================================
-- Based on FK: queue_producer_id -> producer_applications.id
...
-- ============================================
-- Based on FK: user_id -> profiles.id
-- Users see their own, admins see all
