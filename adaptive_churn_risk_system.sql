...
CREATE OR REPLACE FUNCTION get_days_since_last_activity(client_id UUID)
RETURNS INTEGER AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION get_purchase_history_12mo(client_id UUID)
RETURNS TABLE(
...
CREATE OR REPLACE FUNCTION get_client_lifetime_value(client_id UUID)
RETURNS NUMERIC AS $$
DECLARE
    total_value NUMERIC;
BEGIN
...
CREATE OR REPLACE FUNCTION is_seasonal_buyer(client_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
