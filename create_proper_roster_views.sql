-- Create proper roster analytics views with real transaction data
-- This script creates views that pull actual revenue data from existing tables

-- ============================================
-- 1. CREATE PROPER REVENUE VIEWS
-- ============================================
...
SELECT 
    ...
    0 as license_fee_revenue, -- Will be populated when license data is available
    COALESCE(SUM(sp.final_amount), 0) as sync_proposal_revenue,
    COALESCE(SUM(csr.final_amount), 0) as custom_sync_revenue,
    0 as royalty_payment_revenue, -- Will be populated when royalty data is available
    0 as advance_payment_revenue, -- Will be populated when advance data is available
...
SELECT 
    ...
    0 as license_fee_revenue_ytd, -- Will be populated when license data is available
    COALESCE(SUM(sp.final_amount), 0) as sync_proposal_revenue_ytd,
    COALESCE(SUM(csr.final_amount), 0) as custom_sync_revenue_ytd,
    0 as royalty_payment_revenue_ytd, -- Will be populated when royalty data is available
    0 as advance_payment_revenue_ytd, -- Will be populated when advance data is available
    COUNT(DISTINCT CASE WHEN sp.payment_status = 'paid' THEN sp.id END) + 
...
-- ============================================
-- 3. TEST THE VIEWS WITH SAMPLE DATA
-- ============================================
